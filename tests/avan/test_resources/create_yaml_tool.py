...
def create_yaml_LR(file_name, update_dic, abs_path, get_reward=True):
    lock.acquire()
    ...
    if "LR" in update_dic.keys():
        update_dic.update({"LR": round(float(update_dic["LR"]), 9)})

    if "LR" in update_dic.keys():
        doc["model_para"]["actor"]["model_config"]["LR"] = update_dic["LR"]
    if "complete_step" in update_dic.keys():
        doc["agent_para"]["agent_config"]["complete_step"] = update_dic["complete_step"]
    if "size" in update_dic:
        doc["env_para"]["env_info"]["size"] = update_dic["size"]
        doc["env_para"]["env_info"]["wait_num"] = update_dic["wait_nums"]
        doc["env_para"]["env_info"]["vector_env_size"] = update_dic["vector_env_size"]
        doc["env_num"] = update_dic["env_num"]
    if "now_time" in update_dic:
        doc["env_para"].update({"now_time": update_dic["now_time"]})
    
    if "start_core" in update_dic:
        doc["start_core"] = update_dic["start_core"]


    if get_reward:
        if "init_weights" in update_dic:
            doc["model_para"]["actor"]["init_weights"] = update_dic["init_weights"]
    else:
        if "init_weights" in doc["model_para"]["actor"].keys():
            del doc["model_para"]["actor"]["init_weights"]
